...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        self.safety_checker = StableDiffusionSafetyChecker.from_pretrained(
            SAFETY_CACHE, torch_dtype=dtype
        ).to("cuda")
    ...
    def predict(
        ...
        if not disable_safety_checker:
            ...
            for image in output.images:
                _, has_nsfw_concept = self.run_safety_checker(image)
                if not has_nsfw_concept:
                    checked_images.append(image)
            if len(checked_images) < len(output.images):
                print(f"NSFW content detected. {len(output.images) - len(checked_images)} images removed.")
            output.images = checked_images
        ...
        for i, image in enumerate(output.images):
            ...
            image.save(buffered, format=save_format, quality=output_quality if output_format != 'png' else None)

            # Encode the image as a base64 string
            img_str = base64.b64encode(buffered.getvalue()).decode("utf-8")
            
            # Create a data URI
            data_uri = f"data:image/{save_format};base64,{img_str}"
            output_uris.append(data_uri)

        if len(output_uris) == 0:
